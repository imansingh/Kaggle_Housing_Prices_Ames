...
grid.arrange(tr + geom_histogram(aes(x=OverallQualXOverallCond)), 
             te + geom_histogram(aes(x=OverallQualXOverallCond)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=ExterQualXExterCond)), 
             te + geom_histogram(aes(x=ExterQualXExterCond)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=BsmtQualXBsmtCond)), 
             te + geom_histogram(aes(x=BsmtQualXBsmtCond)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=KitchenAbvGrXKitchenQual)), 
             te + geom_histogram(aes(x=KitchenAbvGrXKitchenQual)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=FireplacesXFireplaceQu)), 
             te + geom_histogram(aes(x=FireplacesXFireplaceQu)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=GarageQualXGarageCond)), 
             te + geom_histogram(aes(x=GarageQualXGarageCond)), 
             ncol = 2)

grid.arrange(tr + geom_histogram(aes(x=PoolAreaXPoolQC)), 
             te + geom_histogram(aes(x=PoolAreaXPoolQC)), 
             ncol = 2)
...
prune.ames = prune.tree(tree_model, best = 4)
par(mfrow = c(1, 1))
plot(prune.ames)
...
randomforest_predict_test = predict(randomforest_model, newdata = x_test)
write.csv(x = exp(randomforest_predict_test), file = 'randomforest.csv')
...
predmat = predict(boost.ames, newdata = x_train, n.trees = n.trees)
...
par(mfrow = c(1, 1))
berr = with(x_train, apply((predmat - y)^2, 2, mean))
...
#carried over.
set.seed(0)
boost.ames2 = gbm(y ~ ., data = x_train,
                    distribution = "gaussian",
                    n.trees = 10000,
                    interaction.depth = 15,
                    shrinkage = 0.1)
predmat2 = predict(boost.ames2, newdata = x_train, n.trees = n.trees)

berr2 = with(x_train, apply((predmat2 - y)^2, 2, mean))
